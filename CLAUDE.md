...
vignettes/                # .Rmd vignettes + User-Guide.pdf (R.rsp static vignette)
DESCRIPTION               # Package metadata (AGPL-3 license)
NAMESPACE                 # Auto-generated by roxygen2
```

The User Guide PDF ships as a pre-built (R.rsp `asis`) vignette
(`pkg/vignettes/GlmnetUI-User-Guide.pdf` + `.pdf.asis`); its source stays in
`docs/GlmnetUI-User-Guide.qmd`. When the guide changes, re-render the qmd and
copy the PDF into `pkg/vignettes/`.

## Build & Test

```bash
# Run the Shiny app (port 7879) via devtools
Rscript -e 'devtools::load_all("pkg"); glmnetUI(launch.browser = TRUE)'
