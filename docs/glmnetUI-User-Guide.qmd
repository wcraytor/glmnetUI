...
**3. Variable Configuration** --- Target variable selector, predictor table with checkboxes for Include, Force, and Sign. The Special column (for designating contract dates, coordinates, etc.) appears only in Appraisal and Market modes. See Chapter 6 for full details.
...
**6. Download Output** --- Exports predictions, residuals, CQA scores, and per-variable contributions as an Excel file. Available in all purpose modes.

**7. Download Report** --- Generates a formatted report (Word, PDF, or HTML) saved to the output folder. See Chapter 13. Steps 7--8 (RCA Adjustments and Sales Grid) appear only in Appraisal mode; in General mode the report step is numbered 7.
