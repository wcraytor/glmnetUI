...
glmnetUI is a graphical user interface for the R `glmnet` package, which implements elastic net regularized regression. It runs as a local Shiny application --- there is no login, no server, and no accounts. You launch it from R, import a dataset (CSV or Excel), configure your model, and fit it interactively.

The application provides a complete workflow: data import, variable configuration, model fitting, diagnostic plots, variable importance, model equations, and downloadable reports in Word or PDF format.
...
**4. glmnet Call Parameters** --- All model configuration: alpha, lambda, family, standardize, sign constraints, relaxed lasso, and the interaction matrix. See Chapter 7 for the complete parameter reference.
...
**9. Download Report** --- Generates a formatted report (Word or PDF) saved to the output folder. See Chapter 13.
...
- **Word (.docx)** --- Formatted document via the `officer` package. Suitable for editing and distribution.
- **PDF** --- Generated via `rmarkdown`. Paper size follows the locale setting (Letter or A4).

## Report Contents

Reports include:

- Dataset description and model specification
- Model summary metrics (R\textsuperscript{2}, GR\textsuperscript{2}, CV R\textsuperscript{2}, RMSE, MAE)
- Coefficient table with sign warnings
