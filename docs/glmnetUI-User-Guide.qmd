...
- **RCA computations (appraisal and market modes)** --- After fitting the model, glmnetUI can compute Residual Constraint Approach (RCA) output, which produces per-comparable adjustments, net/gross adjustment summaries, and an adjusted sale price for the subject property.
...
2. **Launch the application**: run `glmnetUI::glmnetUI()` in R. The app opens in your web browser on port 7879. You can also access the app directly by navigating to `http://localhost:7879` in your browser.
...
- **Factor columns**: Text, TRUE/FALSE, and R-factor columns are treated as categorical automatically — a regression can only handle them as factors. A **numeric** column (e.g. a numeric area or style code) becomes a factor only when you tick its **Factor** checkbox; glmnetUI never infers categorical from a numeric's value range, so discrete numeric predictors like `bath_count` stay continuous. The Special role is independent of the factor decision.
...
A **Skip first row** checkbox appears at the top of the sidebar when a General or Market project is open. When checked, row 1 is excluded from model fitting. This is useful when row 1 contains a target or reference observation that should not influence the model. In Appraisal mode, row 1 (the subject property) is always excluded automatically.

### Settings Per Purpose

Settings (predictor selections, model parameters, interactions) are saved separately for each combination of input file and purpose mode. Switching between projects of different purposes preserves each mode's settings independently.
...
**3. Import from earthUI (optional)** --- Import an earthUI result \texttt{.rds} file to use earth's hinge basis functions with glmnet's elastic net regularization. The browse button is styled to match the Section 2 file picker. This step is optional --- skip it if you do not have an earthUI result to import.
...
\hline
Contributions & Per-variable partial effect plots with slope labels. Scatter + line for numeric predictors, box plots for factors, scatter/heatmap/3D surface for interactions. \\
\hline
...
\hline
Glmnet Output & Raw model output: random seed used, model print, selected lambda, gamma, and coefficient vector. \\
\hline
Report & Report configuration and export fields. \\
\hline
RCA Adjustments & RCA analysis results (appraisal/market modes, after computation). \\
\hline
\end{tabularx}
\end{center}
...
glmnetUI automatically saves your configuration to the browser's local storage, keyed by the input filename and purpose. When you reload the same file, all settings are restored: target selection, predictor checkboxes, data types, expected signs, glmnet parameters, and interaction matrix. (The purpose mode itself is set by the active project.) Three options are available:
...
When the active project's purpose is **For Appraisal**, glmnetUI configures itself for single-property valuation. All features described in Chapter 3 remain available; this chapter covers only the appraisal-specific additions.
...
When the active project's purpose is **Market Area Analysis**, glmnetUI provides the same real estate--specific features as appraisal mode (special columns, sale age, coordinate rounding) but is oriented toward analyzing a group of properties rather than valuing a single subject.
...
- **No Sales Grid section** --- the RCA adjustment step (section 8) is available in market mode, but the "Generate Sales Grid \& Download" step is appraisal-only.
...
- Working with a dataset where you want special column features (sale age, coordinate rounding) but do not need the Sales Comparison Grid

\begin{tcolorbox}[colback=nordblue!5, colframe=nordblue!40, title=]
\small\textit{\textbf{Tip:} Market mode is also useful for general real estate regression where you want special column features but do not need the appraisal Sales Comparison Grid.}
\end{tcolorbox}
...
| **Factor** | Checkbox --- treat the column as a categorical factor (creates dummy columns in the model matrix) |
...
- `sale_age` --- Marks an existing sale-age column (days from contract date to effective date); when designated, the automatic `sale_age` computation is skipped and your column is used as-is
- `listing_date` --- Used as a fallback for computing Days on Market
- `dom` --- Identifies the Days on Market column

**Transaction Types:**

- `sale_type` --- Identifies the sale/transaction type column (designation shared with earthUI and mgcvUI)
...
An interactive upper-triangular matrix lets you control which variable pairs are allowed to interact. Each cell contains a checkbox --- checked means the interaction term `x1:x2` is included in the model matrix. All pairs start **unchecked**; when an earthUI model is imported, the pairs used by the earth model are pre-checked.
...
- Right-click again to unblock; the state persists via localStorage per input file

For example, when modeling time adjustments that should scale with property size, block `sale_age` from the main effect and allow it only in an interaction with `living_area`. The resulting `sale_age:living_area` term lets the time appreciation rate vary by house size.

## Random Seed

A **Random seed** text input (pre-filled with a random integer) makes the cross-validation fold assignments reproducible. After each fit, a new seed is auto-generated (using system time, not R's RNG). The last 5 seeds used with the current file are shown as clickable links for easy recall; the seed history resets when a new file is loaded. The seed actually used is reported at the top of the Glmnet Output tab.
...
Section 6 of the sidebar (**Fit Glmnet Model**) contains the **Fit Model** button. Clicking it runs the model with your current configuration.
...
**For factor predictors:** A box plot of contribution values per factor level, sorted by median contribution.

**For two-variable interactions:** A scatter plot colored by the second variable, a heatmap of mean contribution, and a 3D surface plot (interactive plotly when available, otherwise a static `persp()` snapshot).
...
Raw model output for documentation and audit: the random seed used for cross-validation, the printed model object, the selected lambda (with lambda.min/lambda.1se), gamma (if relaxed lasso was used), and the full coefficient vector.

## Report

Configure report fields (appraiser name, property address, report date, file number) and export directly to Word (.docx) or PDF. For HTML output --- or to keep an editable Quarto source --- use the two-step Quarto report flow in sidebar sections 10 and 11. See Chapter 13.
...
After fitting, the **Download Output** button (sidebar section 7, available in all purpose modes) exports an Excel file with predictions and diagnostics to the project's output folder. The section heading reads "Download Estimated Sale Prices \& Residuals" in appraisal/market modes and "Download Estimated Target Variable(s) \& Residuals" in general mode.
...
\hline
\texttt{cqa} & Condition-Quality-Appeal score (0--10 scale) \\
\hline
...
The RCA (Residual Constraint Approach) workflow is available in **Appraisal** and **Market Area Analysis** modes, after fitting the model.

## Opening the RCA Dialog

Click the **Calculate RCA Adjustments \& Download** button in sidebar section 8. Choose:

- **CQA** or **CQA per SF** score type. The "CQA per SF" option appears only when a `living_area` column is designated; it ranks residuals per square foot to reduce the impact of home size.
- Enter the subject's CQA score (0.00--10.00). When an earthUI model has been imported, the score type and value are carried forward from earthUI; otherwise the field starts blank.

Click **Generate** to compute the adjustments and write the Excel output to the project's output folder.
...
## Two-Step Quarto Flow

Report generation is a two-step process in the sidebar:

1. **Generate Quarto Report** (section 10, available once the model is fit) --- writes a self-contained Quarto bundle (\texttt{.qmd} source, plots, \texttt{report\_data.rds}, and \texttt{reference.docx}) to the project's glmnet output folder. Because the bundle is plain Quarto source, you can edit it before rendering.
2. **Convert Quarto Report** (section 11) --- renders a \texttt{.qmd} file to the selected output formats. It defaults to the most recently generated bundle, but you can browse to any \texttt{.qmd}.

The Report tab in the main panel also offers direct **Export to Word (.docx)** and **Export to PDF** downloads using the same report content.

## Report Formats

Three formats are available in the Convert step:

- **HTML** --- Self-contained HTML document. Requires pandoc (bundled with RStudio). Recommended default.
- **Word (.docx)** --- Formatted document. Suitable for editing and distribution. Works on all platforms.
- **PDF** --- Generated via Quarto with LaTeX. Paper size follows the locale setting (Letter or A4). **Requires a LaTeX installation** (see System Requirements appendix). If no LaTeX is detected, the PDF option is automatically hidden.

Reports are rendered using Quarto when available, with a fallback to rmarkdown.
...
A white checkmark appears on the Generate and Convert buttons after successful completion.
...
glmnetUI bundles the same demo dataset used by earthUI in its own `extdata` folder. Locate it with:
