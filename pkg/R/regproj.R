# regProj project model -- shared across the sibling apps (earthUI, mgcvUI,
# glmnetUI) so all three use one project tree and the geo/projects SQLite
# databases at <REGPROJ_ROOT>. Only package-specific bits differ (prefs path,
# reference-cache option name, extdata package, default method). The default
...
#' Path to the per-user glmnetUI preferences file
#'
#' Returns the path to `<R_user_dir("glmnetUI","config")>/prefs.json`. The
...
#' @export
glmnetui_prefs_path <- function() {
  file.path(tools::R_user_dir("glmnetUI", "config"), "prefs.json")
}

#' Read user preferences (returns empty list if file missing)
#' @return Named list.
#' @export
glmnetui_prefs_read <- function() {
  p <- glmnetui_prefs_path()
...
#' @export
glmnetui_prefs_write <- function(prefs) {
...
#'   \item `regproj_root` field in user prefs ([glmnetui_prefs_path()]).
...
regproj_reference <- function() {
  cache <- getOption("glmnetui.regproj_reference_cache", NULL)
  if (!is.null(cache)) return(cache)
  if (!requireNamespace("jsonlite", quietly = TRUE)) {
    stop("Package 'jsonlite' is required.", call. = FALSE)
  }
  path <- system.file("extdata", "regproj_reference.json", package = "glmnetUI")
  if (!nzchar(path) || !file.exists(path)) {
    stop("regproj_reference.json not found in installed package.",
         call. = FALSE)
  }
  ref <- jsonlite::fromJSON(path, simplifyVector = FALSE)
  options(glmnetui.regproj_reference_cache = ref)
  ref
}
...
#'   Defaults to `"glmnet"` for this package.
